  ...
  return (
    <div className='hidden md:block space-y-4 pb-3 md:mb-0'>
      <span className='flex justify-between items-center px-4'>
        <h2 className='font-semibold text-sm'>DIN VARUKORG ({itemCount})</h2>
        <button
...
export function ProductListMobile() {
  ...
  return (
    <>
      <div className='py-2 overflow-hidden  border border-gray-200'>
        <span className='flex justify-between items-center px-4'>
          <h2 className='font-semibold text-sm'>DIN VARUKORG ({itemCount})</h2>
          <button
            className='text-sm  cursor-pointer font-semibold underline underline-offset-2'
      ...
      {/* Render the dropdown when isOpen is true */}
      {isOpen && <ProductListDropdown closeMenu={closeMenu} isOpen={isOpen} />}
    </>
  );
}
